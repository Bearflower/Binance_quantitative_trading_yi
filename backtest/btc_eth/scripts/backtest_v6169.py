"""
v6.16.9方案回测脚本 - 严格均衡版本

核心改进（相对于v6.16.8）：
1. 恢复并收紧SOLUSDT的过滤（回到v6.16.7级别）：
   - ATR绝对下限：0.6%→0.7%
   - ATR强趋势系数：0.9→0.85
   - 成交量S级倍数：1.8→2.0
   - 成交量A级倍数：1.5→1.8
...
4. 提高A级信号质量：
   - 增加A级信号的RSI过滤：多头时4h RSI必须在40-60之间
   - A级最低分：75→78
   - A级仓位系数：30%→25%

5. 增加币种交易比例限制：
   - 最近30笔交易中，单一币种占比超过40%则暂停该币种
...
- A级：币种差异化配置（ETH: 20%, SOL: 25%, 其他: 30%）
- B级：15%
- C级：8%
...
SYMBOL_CONFIG = {
    ...
    'ETHUSDT': {
        ...
        'max_daily_trades': 2,
        'position_ratio_s': 0.5,
        'position_ratio_a': 0.20  # ETH专属A级仓位
    },
    'BNBUSDT': {
        'adx_min': 15,
    ...
    'SOLUSDT': {
        'adx_min': 22,
        'vol_ratio_base': {'S': 2.0, 'A': 1.8, 'B': 0.0, 'C': 0.0},  # v6.16.9收紧
        'atr_abs_min': 0.007,  # v6.16.9: 0.6%→0.7%，收紧
        'atr_percentile': 35,
        'atr_factor_strong': 0.85,  # v6.16.9: 0.9→0.85，收紧
        's_min_score': 88,
        'a_min_score': 78,  # v6.16.9新增
        'max_daily_trades': 1,
        'position_ratio_s': 0.25,
        'position_ratio_a': 0.25  # v6.16.9: 30%→25%，收紧
    },
    'XRPUSDT': {
        'adx_min': 15,
        'vol_ratio_base': {'S': 1.0, 'A': 1.0, 'B': 0.0, 'C': 0.0},
...
class BacktestEngine:
    ...
    def __init__(self, config: Dict):
        ...
        # v6.16.9新增：币种交易比例限制
        self.recent_trades_window = deque(maxlen=30)  # 最近30笔交易
        ...
        self.filter_stats = {
            ...
            'symbol_ratio_filtered': 0,  # v6.16.9新增
            'opened_positions': 0,
    ...
    def check_symbol_ratio_limit(
        self,
        symbol: str
    ) -> Tuple[bool, str]:
        """
        检查币种交易比例限制（v6.16.9新增）
        最近30笔交易中，单一币种占比超过40%则暂停该币种
        """
        if len(self.recent_trades_window) < 10:
            return True, "交易记录不足，跳过比例检查"

        # 统计最近30笔交易中各币种的占比
        symbol_count = {}
        for s in self.recent_trades_window:
            symbol_count[s] = symbol_count.get(s, 0) + 1

        total_count = len(self.recent_trades_window)
        symbol_ratio = symbol_count.get(symbol, 0) / total_count

        if symbol_ratio > 0.4:
            return False, f"{symbol}交易占比过高（{symbol_ratio*100:.1f}% > 40%），暂停交易"

        return True, f"{symbol}交易占比正常（{symbol_ratio*100:.1f}% ≤ 40%）"
    ...
    def _check_and_open_position(
        ...
        a_min_score = symbol_config.get('a_min_score', 75)
        thresholds = {
            'S': s_min_score,
            'A': a_min_score,
            'B': 75,
        ...
        # 11. v6.16.9新增：币种交易比例限制
        ratio_ok, ratio_reason = self.check_symbol_ratio_limit(symbol)
        if not ratio_ok:
            self.filter_stats['symbol_ratio_filtered'] += 1
            logger.debug(
                f"币种交易比例限制",
                symbol=symbol,
                time=current_time,
        ...
        if grade == 'S':
            ...
            position_ratio = Decimal(str(symbol_config.get('position_ratio_a', 0.30)))
        else:
        ...
        # v6.16.9新增：更新最近交易窗口
        self.recent_trades_window.append(symbol)
    ...
    def check_grade_a_filters(
        ...
        """
        A级信号额外过滤（v6.16.9增强）
        1. 多头时4h RSI必须在40-60之间
        2. MACD柱必须同向
        """
        if grade != 'A':
            return True, "非A级信号，跳过A级过滤"

        # v6.16.9新增：多头时4h RSI必须在40-60之间
        if pd.notna(rsi_4h):
            if direction == 'LONG':
                if rsi_4h < 40 or rsi_4h > 60:
                    return False, f"A级做多RSI不在40-60区间（RSI_4h={rsi_4h:.1f}）"
            elif direction == 'SHORT':
                if rsi_4h > 70:
                    return False, f"A级做空RSI过高（RSI_4h={rsi_4h:.1f} > 70）"
                elif rsi_4h < 30:
...
def main():
    ...
    print(f"- 币种交易比例过滤: {results['filter_stats']['symbol_ratio_filtered']}")
    print(f"- S级降级: {results['filter_stats']['grade_s_downgraded']}")
    print(f"- A级RSI过滤: {results['filter_stats']['grade_a_rsi_filtered']}")
    print(f"- A级MACD过滤: {results['filter_stats']['grade_a_macd_filtered']}")
