...
- ADX ≥ 15
- ATR%范围：1.5%-6.0%
- 成交量倍数：S≥1.5，A≥1.3，B/C≥1.0
- RSI过滤：A级信号要求4h RSI在35-65之间
...
class BacktestEngine:
    ...
    def __init__(self, config: Dict):
        ...
        self.tp1_atr_multiplier = Decimal('3.0')  # TP1：3.0×ATR
        self.tp2_atr_multiplier = Decimal('4.5')  # TP2：4.5×ATR
        self.trailing_activation_atr = Decimal('2.5')  # 吊灯启动：2.5×ATR
        self.trailing_stop_atr = Decimal('1.8')  # 吊灯回撤：1.8×ATR
        self.time_stop_hours = 72  # 时间止损：72小时

        # TP1和TP2平仓比例
        self.tp1_close_ratio = Decimal('0.20')  # TP1平仓20%
        self.tp2_close_ratio = Decimal('0.30')  # TP2平仓30%
    ...
    def is_trend_aligned_v615(
        ...
        日线要求：close > ema21
        4小时允许偏离：tolerance = 0.5 * atr_4h / h4_close
        ...
        # 计算4小时ATR
        atr_4h = Decimal(str(df_4h['close'].rolling(window=14).apply(
            lambda x: max(x) - min(x)
        ).iloc[-1]))

        # 计算允许偏离比例
        tolerance = Decimal('0.5') * atr_4h / close_4h if close_4h > 0 else Decimal('0')

        # 日线趋势判断
        daily_ok = close_1d > ema21_1d_value if direction == 'LONG' else close_1d < ema21_1d_value

        if not daily_ok:
            return False, f"日线趋势不一致（{'多头需close>ema21' if direction == 'LONG' else '空头需close<ema21'}）"

        # 4小时趋势判断（允许偏离）
        if direction == 'LONG':
            # 多头：close >= ema21 * (1 - tolerance)
            h4_ok = close_4h >= ema21_4h_value * (Decimal('1') - tolerance)
            if not h4_ok:
                return False, f"4小时趋势偏离过大（close={close_4h:.2f}, ema21={ema21_4h_value:.2f}, 允许偏离{float(tolerance)*100:.2f}%）"
        else:
            # 空头：close <= ema21 * (1 + tolerance)
            h4_ok = close_4h <= ema21_4h_value * (Decimal('1') + tolerance)
            if not h4_ok:
                return False, f"4小时趋势偏离过大（close={close_4h:.2f}, ema21={ema21_4h_value:.2f}, 允许偏离{float(tolerance)*100:.2f}%）"
    ...
    def check_rsi_filter_v615(
        ...
        """
        if pd.isna(rsi_4h):
            return False, "RSI数据缺失"

        # A级信号特殊要求
        if grade == 'A':
            if not (35 <= rsi_4h <= 65):
                return False, f"A级信号RSI={rsi_4h:.2f}不在35-65区间"
    ...
    def get_volume_multiplier(self, grade: str) -> float:
        ...
        return volume_multipliers.get(grade, 1.0)
    ...
    def _check_and_open_position(
        ...
        # 6. 趋势方向一致性过滤（v6.15方案A）
        trend_aligned, trend_reason = self.is_trend_aligned_v615(klines_4h, klines_1d, direction)
        if not trend_aligned:
            self.filter_stats['trend_alignment_filtered'] += 1
            return

        # 7. RSI过滤（v6.15：A级信号特殊要求）
        rsi_4h = indicators_4h['RSI'].iloc[-1]
        rsi_ok, rsi_reason = self.check_rsi_filter_v615(rsi_4h, grade)
        if not rsi_ok:
...
1. **趋势一致性过滤（方案A）**
   ...
   - 4小时：允许偏离，tolerance = 0.5 × ATR / close
