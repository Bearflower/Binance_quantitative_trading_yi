...
1. SOLUSDT微调：
   - ATR绝对下限：0.7%→0.6%
   - 其他参数保持严格（ADX=22, 成交量S=2.0/A=1.8, S级最低分=88）
...
# 币种差异化配置（v6.16.10微调版本）
SYMBOL_CONFIG = {
    'BTCUSDT': {
        'adx_min': 12,  # v6.16.10: 放宽 15→12
        'vol_ratio_base': {'S': 1.2, 'A': 1.0, 'B': 0.0, 'C': 0.0},  # v6.16.10: 放宽
        'atr_abs_min': 0.005,   # v6.16.10: 放宽 0.6%→0.5%
        'atr_percentile': 30,
        'atr_factor_strong': 0.7,
        's_min_score': 82,  # v6.16.10: 放宽 85→82
        'a_min_score': 75,
        'max_daily_trades': 2,
    ...
    'SOLUSDT': {
        'adx_min': 22,
        'vol_ratio_base': {'S': 2.0, 'A': 1.8, 'B': 0.0, 'C': 0.0},  # 保持严格
        'atr_abs_min': 0.006,  # v6.16.10: 微调 0.7%→0.6%
        'atr_percentile': 35,
        'atr_factor_strong': 0.85,  # 保持严格
        's_min_score': 88,
        'a_min_score': 78,
        'max_daily_trades': 1,
        'position_ratio_s': 0.25,
        'position_ratio_a': 0.25
    },
    'XRPUSDT': {
        'adx_min': 15,
        'vol_ratio_base': {'S': 1.0, 'A': 1.0, 'B': 0.0, 'C': 0.0},
...
class BacktestEngine:
    ...
    def __init__(self, config: Dict):
        ...
        # v6.16.10: 全局每日最大交易数限制
        self.global_max_daily_trades = 6  # 所有币种合计每日最多6笔
        ...
        self.filter_stats = {
            ...
            'grade_a_rsi_adjusted': 0,  # v6.16.10: 改为调整统计
    ...
    def adjust_score_for_rsi(
        ...
        """
        if grade != 'A':
            return score, "非A级信号，不调整"
        
        if pd.isna(rsi_4h):
            return score, "RSI数据缺失，不调整"
        
        # RSI在35-65之间：+2分
        if 35 <= rsi_4h <= 65:
            adjusted_score = score + 2
            return adjusted_score, f"A级RSI加分（RSI_4h={rsi_4h:.1f}在35-65区间，+2分）"
        else:
            # RSI不在35-65之间：-2分
            adjusted_score = score - 2
            return adjusted_score, f"A级RSI减分（RSI_4h={rsi_4h:.1f}不在35-65区间，-2分）"
    ...
    def _check_and_open_position(
        ...
        # 如果是A级，应用RSI评分调整
        if temp_grade == 'A':
            score, adjust_reason = self.adjust_score_for_rsi(score, rsi_4h, 'A')
            self.filter_stats['grade_a_rsi_adjusted'] += 1
            logger.debug(
                f"A级RSI评分调整",
                symbol=symbol,
                time=current_time,
                reason=adjust_reason,
                original_score=score - (2 if 35 <= rsi_4h <= 65 else -2),
        ...
        # 10. A级MACD过滤
        if grade == 'A' and pd.notna(macd_hist):
    ...
    def check_cooldown_and_daily_limit(
        ...
        # v6.16.10: 检查全局每日限制（所有币种合计）
        total_today = 0
        if today in self.daily_trade_count:
            for sym, count in self.daily_trade_count[today].items():
                total_today += count
...
def main():
    ...
    print(f"- A级RSI调整: {results['filter_stats']['grade_a_rsi_adjusted']}")
