"""
v6.16.6方案回测脚本 - 精准参数优化版

核心改进（相对于v6.16.5）：
1. S级收紧：评分阈值85→88，仓位系数50%→35%
2. C级提仓：仓位系数5%→8%
3. 止损放宽：1.8×ATR→2.2×ATR
4. 时间止损加速：72h→48h
5. ETH专属降仓：A级30%→20%
6. 动态成交量过滤器保持不变
...
# 所有币种列表（v6.16.5：取消高流动性跳过，统一使用ADX-based动态阈值）
...
class BacktestEngine:
    ...
    def __init__(self, config: Dict):
        ...
        # v6.16.6止盈止损参数
        self.stop_loss_atr = Decimal('2.2')  # 止损：2.2×ATR（v6.16.6：1.8→2.2，减少噪音扫损）
        self.tp1_atr_multiplier = Decimal('2.5')  # TP1：2.5×ATR
        self.tp2_atr_multiplier = Decimal('4.0')  # TP2：4.0×ATR
        self.trailing_activation_atr = Decimal('1.8')  # 吊灯启动：1.8×ATR
        self.trailing_stop_atr = Decimal('1.2')  # 吊灯回撤：1.2×ATR
        self.time_stop_hours = 48  # 时间止损：48小时（v6.16.6：72→48）
        ...
        # v6.16.6等级阈值（S级从85提高到88）
        self.grade_thresholds = {
            'S': 88,  # v6.16.6：85→88，减少劣质S级
            'A': 75,
            'B': 75,
            'C': 55
        }
    ...
    def check_volume_filter(
        ...
        """
        动态成交量过滤检查（v6.16.5核心优化：方案A ADX-based动态阈值）

        核心理念：趋势越强，回调时缩量越正常，阈值越宽松。
        所有币种统一使用ADX-based动态阈值，不再区分高流动性币种。
        ...
        if not dynamic_config.get('enabled', False):
            threshold = self.config['strategy']['filters'].get('volume_multipliers', {}).get('A', 1.3)
        elif adx_1d is None or pd.isna(adx_1d):
        ...
        volume_ratio = volume / volume_ma if pd.notna(volume) and pd.notna(volume_ma) and volume_ma > 0 else 0
    ...
    def _check_and_open_position(
        ...
        # 7. 成交量过滤（v6.16.5：ADX-based动态阈值，所有币种统一）
        volume = indicators_1h['volume'].iloc[-1]
        volume_ma = indicators_1h['Volume_MA'].iloc[-1]
        adx_1d = indicators_1d['ADX'].iloc[-1] if 'ADX' in indicators_1d.columns else None
        ...
        # 计算仓位（v6.16.6：支持ETH专属仓位覆盖 + 成交量调整系数）
...
def print_report(results: Dict, symbols: List[str]):
    ...
    """
    print(f"""
# v6.16.6方案回测结果 - 精准参数优化版

## 一、方案参数配置

### 核心改进（相对于v6.16.5）
1. **S级收紧**
   - 评分阈值：85→88（↑3分，减少劣质S级信号）
   - 仓位系数：50%→35%（↓15%，降低亏损源影响）

2. **C级提仓**
   - 仓位系数：5%→8%（↑3%，提升正收益贡献）

3. **止损放宽**
   - 固定止损：1.8×ATR→2.2×ATR（↑0.4，减少噪音扫损）

4. **时间止损加速**
   - 最大持仓：72h→48h（↓24h，更快释放资金）

5. **ETH专属降仓**
   - ETH的A级仓位：30%→20%（针对ETH胜率高但亏损的问题）

6. **TP参数优化**
   - TP1：3.5→2.5×ATR，TP2：6.0→4.0×ATR（降低止盈目标，提高命中率）
   - 吊灯启动：2.0→1.8×ATR，吊灯回撤：1.5→1.2×ATR（更早保护利润）

7. **动态成交量过滤器保持不变**

### 参数变更汇总表
| 参数 | v6.16.5 | v6.16.6 | 变化 |
|------|:---:|:---:|:---:|
| S级评分阈值 | 85 | 88 | +3 |
| S级仓位系数 | 50% | 35% | -15% |
| C级仓位系数 | 5% | 8% | +3% |
| ETH A级仓位 | 30% | 20% | -10% |
| 止损ATR倍数 | 1.8 | 2.2 | +0.4 |
| TP1 ATR倍数 | 3.5 | 2.5 | -1.0 |
| TP2 ATR倍数 | 6.0 | 4.0 | -2.0 |
| 吊灯启动ATR | 2.0 | 1.8 | -0.2 |
| 吊灯回撤ATR | 1.5 | 1.2 | -0.3 |
| 时间止损 | 72h | 48h | -24h |
