"""
v6.16.5方案回测脚本 - 动态成交量过滤器版（方案A：ADX-based动态阈值）
...
def print_report(results: Dict, symbols: List[str]):
    ...
    """
    print(f"""
# v6.16.5方案回测结果 - 动态成交量过滤器版（方案A：ADX-based动态阈值）
...
3. **策略代码同步更新**
   - strategy.py 新增 `_get_dynamic_volume_threshold()` 方法
   - _check_pre_filters 集成ADX_1d外传和动态阈值计算
