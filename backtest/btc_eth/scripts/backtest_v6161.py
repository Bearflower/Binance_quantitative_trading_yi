...
class BacktestEngine:
    ...
    def __init__(self, config: Dict):
        ...
        # v6.16.1频率控制参数（优化四：冷却期缩短至4小时）
        self.max_daily_trades = 6  # 每日最大交易数
        self.symbol_cooldown_hours = 4  # 同币种冷却期（从6小时缩短至4小时）
    ...
    def check_grade_a_filters(
        ...
        # 成交量过滤：≥1.3倍（v6.16.1降低要求）
        if volume_ratio < 1.3:
            return False, f"A级信号成交量不足（{volume_ratio:.2f}倍 < 1.3倍）"
    ...
    def _check_and_open_position(
        ...
        # 2. ATR%范围过滤：1.0%-8.5%（优化一：上限从7.0%放宽至8.5%）
        ...
        # 6. 成交量放大过滤（v6.16.1：S≥1.4，A≥1.3，B/C不要求）
