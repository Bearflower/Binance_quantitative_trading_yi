...
2. 非BTC币种成交量放宽：
   - ETH, BNB, SOL, XRP, TRX：取消成交量要求
   - BTC：保留1.2倍软要求（若低于阈值，仓位系数减半，不直接拒绝）
...
4. 支持多币种回测（BTCUSDT, ETHUSDT, BNBUSDT, XRPUSDT, SOLUSDT, TRXUSDT）

前置过滤器：
- ADX ≥ 15
- ATR%范围：0.3%-8.5%（降低下限以适应低波动市场）
- 成交量倍数：动态计算
- 同币种冷却期：4小时
- 每日最大交易数：6笔
