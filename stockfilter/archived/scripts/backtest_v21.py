...
# -*- coding: utf-8 -*-
"""
V2.1 回测器执行脚本（使用 baostocks_full 完整数据）
"""
...
def run_backtest_v21(
    ...
    """运行 V2.1 回测"""
    logger.info("="*80)
    logger.info("开始 V2.1 回测（缩量到放量时间窗口：10 天）")
    logger.info("="*80)
    ...
    # 创建回测器（使用 V2.1，即原始的 backtester_scheme_ab）
    ...
    # 保存 JSON
    json_file = output_dir / f'backtest_v21_baostocks_{timestamp}.json'
    ...
    # 保存 CSV
    if all_trades:
        csv_file = output_dir / f'backtest_v21_baostocks_{timestamp}.csv'
