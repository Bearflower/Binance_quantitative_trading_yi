...
"""
V2.3 回测器：
- 进一步放宽缩量到放量的时间窗口（从 25 天延长到 40 天）
- 适当放宽其他条件以增加信号数量
...
class BacktesterV23:
    ...
    def __init__(self, config_path: str = 'config_v21_final.yaml'):
        ...
        # V2.3 新增：进一步放宽缩量到放量的时间窗口
        self.shrink_to_surge_days = 40  # V2.3: 40 天（从 25 天放宽）
        
        # V2.3 放宽其他条件
        self.drop_threshold = 0.10  # 跌幅从 12% 放宽到 10%
        self.surge_price_ratio = 0.04  # 涨幅从 5% 放宽到 4%
        self.min_volume_ratio = 1.3  # 量比从 1.5 放宽到 1.3
        self.volume_shrink_ratio = 0.7  # 缩量从 60% 放宽到 70%（成交量可以更高）
