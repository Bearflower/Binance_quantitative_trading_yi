...
echo ""
echo "1. 查看实时日志："
echo "   ssh -i /Users/yl/vscode/inspection_automation/docs/only.pem root@${SERVER_HOST} \"docker logs -f ${CONTAINER_NAME}\""
echo ""
echo "2. 手动触发股票列表同步（可选）："
echo "   ssh -i /Users/yl/vscode/inspection_automation/docs/only.pem root@${SERVER_HOST} \"docker exec ${CONTAINER_NAME} python main.py --init\""
echo ""
echo "3. 验证数据库中的股票数量："
echo "   ssh -i /Users/yl/vscode/inspection_automation/docs/only.pem root@${SERVER_HOST} \"docker exec ${CONTAINER_NAME} python -c \\\"from data.database import DatabaseManager; db = DatabaseManager(); stocks = db.get_stock_list(); print(f'股票总数：{len(stocks)}')\\\"\""
echo ""
echo "✅ 修复说明："
