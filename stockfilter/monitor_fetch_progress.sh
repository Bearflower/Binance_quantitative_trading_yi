...
# 检查后台进程
echo "【后台进程状态】"
ps_count=$(ssh -i /Users/yl/vscode/inspection_automation/docs/only.pem root@43.156.242.184 "ps aux | grep -E 'python.*fetch|python.*DatabaseManager' | grep -v grep | wc -l")
...
# 查询总记录数
total_records=$(ssh -i /Users/yl/vscode/inspection_automation/docs/only.pem root@43.156.242.184 "docker exec -i stockfilter-app python3 -c \"from data.database import DatabaseManager; db = DatabaseManager(); import pandas as pd; df = pd.read_sql('SELECT COUNT(*) as cnt FROM klines', db.conn); print(df.iloc[0,0]); db.close()\" 2>/dev/null")
