...
# 检查进程
ps_count=$(ssh -i ~/.ssh/stockfilter_key root@43.156.242.184 "ps aux | grep python | grep -v grep | wc -l")
echo "【进程状态】"
...
# 查询总记录数
total=$(ssh -i ~/.ssh/stockfilter_key root@43.156.242.184 "docker exec -i stockfilter-app python3 -c \"from data.database import DatabaseManager; db = DatabaseManager(); import pandas as pd; df = pd.read_sql('SELECT COUNT(*) as cnt FROM klines', db.conn); print(df.iloc[0,0]); db.close()\" 2>/dev/null")
