...
class CompleteBacktesterV4Adjusted:
    """V4.0 完整回测器（调整版）"""
            ...
            for i in range(10, min(len(klines_1h), 100)):
                ...
                for j in range(i+1, min(i+1+self.max_holding_hours, len(klines_1h))):
                    ...
                    if future_high >= stop_loss_price:
                        ...
                        exit_reason = "止损"
                        trade_exit = {
                            'exit_time': datetime.fromisoformat(future_kline.get('timestamp', '')),
                ...
                if trade_exit is None:
                    ...
                    exit_reason = "时间止损"
                    trade_exit = {
                        'exit_time': datetime.fromisoformat(last_kline.get('timestamp', '')),
                        'exit_price': exit_price,
                        'pnl': pnl,
                        'exit_reason': exit_reason
                    }
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='V4.0 完整回测（调整版）')
    parser.add_argument('--data', type=str, default='data/2025_new_coins_data.json')
    parser.add_argument('--output', type=str, default='data/backtest_v4_complete_adjusted.json')
    parser.add_argument('--threshold', type=float, default=6.0)

    args = parser.parse_args()

    backtester = CompleteBacktesterV4Adjusted(entry_threshold=args.threshold)
