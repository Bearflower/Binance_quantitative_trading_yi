...
"""
在服务器上运行的数据获取脚本（增强版）
获取币安 12 个指定币种的多时间框架数据
包含：1d, 4h, 1h, 15m, 资金费率，OI
"""
...
def fetch_klines(symbol: str, interval: str = '1h', limit: int = 1000) -> List[Dict]:
    """
    从币安获取 K 线数据
    注意：币安 API limit 最大值为 1000
    """
...
def fetch_multi_timeframe_data(symbols: List[str], days: int = 90) -> Dict:
    """获取多时间框架数据（包含 15 分钟）"""
    klines_1d = days
    klines_4h = days * 6
    klines_1h = days * 24
    klines_15m = days * 96  # 15 分钟 K 线
    ...
    for symbol in symbols:
        ...
        symbol_data = {
            ...
            '15m': fetch_klines(symbol, interval='15m', limit=klines_15m),
        ...
        print(f"  ✅ 15m: {len(symbol_data['15m'])} 条")
...
def main():
    ...
    parser = argparse.ArgumentParser(description='获取币安回测数据（服务器版，含 15 分钟）')
    parser.add_argument('--symbols', type=str, default=','.join(TARGET_SYMBOLS))
    parser.add_argument('--days', type=int, default=90)
    parser.add_argument('--output', type=str, default='/root/short_selling_system/data/backtest_data.json')
    ...
    print("=" * 80)
    print("币安回测数据获取工具（增强版 - 含 15 分钟）")
    print("=" * 80)
    print(f"目标币种：{len(symbols)} 个")
    for s in symbols:
        print(f"  - {s}")
    print(f"数据天数：{args.days} 天")
    print(f"时间框架：1d, 4h, 1h, 15m")
    print("=" * 80)
    ...
    total_klines = sum(
        len(d['1d']) + len(d['4h']) + len(d['1h']) + len(d['15m'])
    ...
    print(f"\n从本地下载数据:")
    print(f"  cd /Users/yl/vscode/bianace_newtrade_trade/short_selling_system")
