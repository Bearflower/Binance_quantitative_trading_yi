...
class CompleteBacktesterV4NoPause:
    """V4.0 完整回测器（无暂停限制版）"""
    ...
    def __init__(
        ...
        print(f"✅ V4.0 完整回测器初始化完成（无暂停限制版）")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='V4.0 完整回测（无暂停限制版）')
    parser.add_argument('--data', type=str, default='data/2025_new_coins_data.json')
    parser.add_argument('--output', type=str, default='data/backtest_v4_no_pause.json')
    parser.add_argument('--threshold', type=float, default=6.0)
    parser.add_argument('--max-klines', type=int, default=100)

    args = parser.parse_args()

    backtester = CompleteBacktesterV4NoPause(
