...
class CompleteBacktesterV4RealData:
    """V4.0 完整回测器（使用真实数据）"""
    ...
    def __init__(
        ...
        print(f"✅ V4.0 完整回测器初始化完成（使用真实数据）")
    ...
    def run_backtest(self, kline_data_file: str, real_data_file: str, output_file: str):
        ...
        for symbol, symbol_data in symbols_data.items():
            ...
            oi_ratio = symbol_real_data.get('oi_ratio')
            funding_rate = symbol_real_data.get('funding_rate')

            # 如果没有OI/市值比或资金费率数据，跳过
            if oi_ratio is None or funding_rate is None:
                continue
            ...
            for i in range(10, min(len(klines_1h), self.max_klines_per_symbol)):
                ...
                # 使用真实的OI/市值比和资金费率
                contract_score, _ = scoring_engine_v4.calculate_contract_score(oi_ratio)
                ...
                print(f"     OI/市值比: {oi_ratio:.4f}, 资金费率: {funding_rate:.2f}%")
...
if __name__ == '__main__':
    ...
    parser = argparse.ArgumentParser(description='V4.0 完整回测（使用真实数据）')
    ...
    parser.add_argument('--output', type=str, default='data/backtest_v4_real_data.json')
    parser.add_argument('--threshold', type=float, default=6.0)
    parser.add_argument('--max-klines', type=int, default=100)

    args = parser.parse_args()

    backtester = CompleteBacktesterV4RealData(
