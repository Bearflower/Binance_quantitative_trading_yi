...
class BacktestV411:
    def __init__(self):
        ...
        print("✅ V4.1.1 回测引擎初始化完成")
        print(f"   止损: {self.config['stop_loss_atr']} ATR")
        print(f"   止盈: {self.config['take_profit_atr']} ATR")
        print(f"   盈亏比: {self.config['take_profit_atr']/self.config['stop_loss_atr']:.2f}")
    ...
    def analyze_results(self, trades: List[BacktestTrade]) -> Dict:
        ...
        return {
            ...
            'config': {
                ...
                'risk_reward_ratio': self.config['take_profit_atr'] / self.config['stop_loss_atr']
            },
...
def main():
    print("=" * 60)
    print("V4.1.1 回测 - 止盈2.5 ATR，止损2.0 ATR")
    print("=" * 60)
    ...
    print(f"止盈: {results['config']['take_profit_atr']} ATR")
    print(f"盈亏比: {results['config']['risk_reward_ratio']:.2f}")
    print("-" * 60)
    ...
    output_file = "/Users/yl/vscode/bianace_newtrade_trade/short_selling_system/data/backtest_v411_results.json"
    with open(output_file, 'w') as f:
        json.dump({
            'metadata': {'version': 'V4.1.1', 'backtest_time': datetime.now().isoformat()},
            'summary': results,
            'trades': [
                {
                    'symbol': t.symbol, 'entry_time': t.entry_time.isoformat(),
                    'entry_price': t.entry_price, 'exit_time': t.exit_time.isoformat(),
                    'exit_price': t.exit_price, 'pnl_usd': t.pnl_usd,
                    'exit_reason': t.exit_reason, 'holding_hours': t.holding_hours
                }
                for t in trades
            ]
