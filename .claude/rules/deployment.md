...
if echo "$CHANGED_FILES" | grep -qv "^shared/\|^backtest/\|^docs/\|^.trae/\|^.claude/\|^.git"; then
    AFFECTED_SERVICES+=(hrs-strategy)
fi
...
- **server-deployment（服务器自动化部署）** — 详细的部署流程和脚本（`.claude/skills/server-deployment/SKILL.md`）
- **common-modules（通用模块调用指南）** — K线服务、通知服务等通用模块的使用（`.claude/skills/common-modules/SKILL.md`）
