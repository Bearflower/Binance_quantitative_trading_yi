...
from strategies.btc_eth_aggressive.market_state import (
    get_market_state,
...
class FrequencyController:
    ...
    def __init__(self, config: Dict, db_manager=None, strategy_name: str = "MTPCS激进策略"):
        """
        初始化频率控制器
...
class BTCEthStrategy:
    ...
    def __init__(
        ...
        self.strategy_name = "btc_eth_aggressive"
        ...
        # 激进版独立参与月度资金分配（participating_strategies 含 btc_eth_aggressive），
        # strategy_id 使用 btc_eth_aggressive，不再与原版共用分配结果。
    ...
    def _check_ranging_entry(
        ...
        try:
            ...
            # (1) 开仓侧三机制总开关（entry_filters.enabled，v6.26 扩展）
            # 关闭时跳过波动突变/方向一致性/量价确认/重度过热四机制，恢复 v6.25 宽松开仓，
            # 仅凭三条件投票得到的方向直接放行（激进版通过 config 显式设置为 false）。
            # 注意：时间平仓复核 _should_keep_position 不受此开关影响，仍用各机制自身 enabled。
            entry_cfg = ranging_config.get('entry_filters', {})
            if not entry_cfg.get('enabled', True):
                logger.info(f"{symbol} 震荡入场通过（开仓侧三机制已关闭）", direction=direction,
    ...
    def _calc_current_total_margin(self) -> float:
        """统计 MTPCS(aggressive) 策略当前全部持仓的保证金总和（统一保证金口径）
    ...
    def build_positions_report(self) -> tuple:
        """构建 MTPCS(aggressive) 策略持仓上报数据（供 main.py 统一调用）

        与 btc_eth 策略保持同一口径：同一套持仓快照在"开仓成功后"与"周期末"
        两处复用，避免重复构造。返回三份字典：
        - positions:    strategy_states 表持仓快照（含方向/入场价/数量/订单ID等）
        - margin_dict:  {symbol: 保证金(USDT)}，仅对 数量>0 且 有入场价 的持仓计算
        - qty_dict:     {symbol: 当前持仓数量}

        杠杆口径与 _calc_current_total_margin 保持一致：
    ...
    async def _do_startup_orphan_cleanup(self):
        ...
        try:
            ...
            open_orders = await get_open_orders(self.db_manager, "btc_eth_aggressive")
            if not open_orders:
