...
### 豁免场景：AI 自动化调优流水线中的轻量级回测

以下场景的回测**不受上述规则限制**，允许在服务器（ai-tuner 容器内）执行：

1. **StratTuneAI 周度调优流水线中的回测**：作为 AI 调优的输入数据，回测在 ai-tuner 容器内执行
2. **单策略、单周数据、单次运行**：回测范围仅限于单个策略的一周数据（如 168 根 1h K线）
3. **计算量极低**：单次回测耗时 < 10ms，内存消耗 < 200KB，不会影响生产交易程序

豁免条件：
- 回测代码必须部署在 ai-tuner 容器内，不得在策略容器（btc_eth、new_coin、grid、hrs）内执行
- 回测不得使用多进程/多线程并行
- 回测数据量不得超过 500 根 K线

```bash
# ✅ 允许：ai-tuner 容器内的自动化回测（AI 调优流水线的一部分）
# 在 ai_tuner/backtest/ 模块中，由 weekly_job.py 调度执行

# ❌ 禁止：在策略容器内执行回测
# ❌ 禁止：手动 SSH 到服务器执行回测脚本
# ❌ 禁止：大规模历史数据回测（> 500 根 K线）
```
