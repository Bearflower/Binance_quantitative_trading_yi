...
logger = logging.getLogger('v613_real_backtest')


class V613RealBacktester:
    """V6.13 真实回测器（使用多时间框架数据）"""
    ...
    def __init__(self, initial_capital: Decimal = Decimal('500')):
        ...
        self.atr_config = {
            ...
            'tp1_atr': Decimal('4.0'),      # 原版：4.0
            'tp2_atr': Decimal('6.0'),      # 原版：6.0
            'tp1_ratio': Decimal('0.25'),
            'tp2_ratio': Decimal('0.25'),
            'chandelier_start_atr': Decimal('2.5'),  # 原版：2.5
            'chandelier_pullback_atr': Decimal('1.5'),  # 原版：1.5
        }
        ...
        logger.info("=" * 80)
        logger.info("V6.13 真实回测器初始化完成")
        logger.info("=" * 80)
        logger.info(f"初始资金：{initial_capital}U")
        logger.info(f"止盈参数：TP1={self.atr_config['tp1_atr']}×ATR, TP2={self.atr_config['tp2_atr']}×ATR")
    ...
    def run_backtest(self, signals: List[Dict[str, Any]]) -> Dict[str, Any]:
        ...
        return {
            'strategy': 'V6.13 真实回测（多时间框架 + 原版止盈止损）',
...
def main():
    """主函数"""
    logger.info("开始 V6.13 真实回测")
    
    # 1. 初始化回测器
    backtester = V613RealBacktester(initial_capital=Decimal('500'))
    ...
    # 5. 保存报告
    report_file = f'data/backtest_v613_real_{datetime.now().strftime("%Y%m%d_%H%M%S")}.json'
