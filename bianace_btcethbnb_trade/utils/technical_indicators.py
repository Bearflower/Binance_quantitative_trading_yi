...
def get_open_interest(symbol="BTCUSDT"):
    ...
    try:
        ...
        if response.status_code == 200:
            ...
            print(f"获取持仓量失败：{response.status_code}")
            return None
    except Exception as e:
        print(f"获取持仓量错误：{str(e)}")
        return None
...
def get_technical_indicators(symbol="BTCUSDT"):
    ...
    if open_interest:
        ...
        print(f"警告：获取 {symbol} 持仓量数据失败")
        indicators["open_interest"] = None
    
    # 获取订单簿深度
    order_book = get_order_book(symbol, 20)
    if order_book:
        indicators["order_book"] = {
            "bids": order_book["bids"][:5],  # 前 5 个买单
            "asks": order_book["asks"][:5]   # 前 5 个卖单
        }
