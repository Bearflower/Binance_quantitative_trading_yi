...
from core.scoring_engine_v612 import get_scoring_engine_v612
...
class RealDataBacktesterV6:
    ...
    def __init__(self, data_file: str = 'data/multi_timeframe_data.json'):
        ...
        # v6.12 评分阈值（提高质量）
        self.min_score_s = 85        # S 级≥85 分
        self.min_score_a = 75        # A 级 70→75（恢复标准）
        self.min_score_b = 65        # B 级 60→65（提高质量）
        self.min_score_c = 55        # C 级 50→55（提高质量）
        
        # v6.12 频率控制（稳健型）
        self.max_trades_per_day = 4  # v6.11: 5 → v6.12: 4（减少过度交易）
        self.cooldown_hours = 12     # v6: 24 → v6.11/12: 12
        self.max_consecutive_losses = 5  # 连续 5 笔亏损暂停
        ...
        logger.info("="*60)
        logger.info("回测器 v6.12 初始化完成（方案 A - 稳健型）")
        logger.info("="*60)
        logger.info(f"✅ 保留 v6 原版前置过滤器（ADX/成交量/ATR）")
        logger.info(f"✅ B 级阈值：60→65 分（提高质量）")
        logger.info(f"✅ C 级阈值：50→55 分（提高质量）")
        logger.info(f"✅ S 级阈值：≥{self.min_score_s}分")
        logger.info(f"✅ A 级阈值：≥{self.min_score_a}分（70→75）")
        logger.info(f"✅ 频率限制：{self.max_trades_per_day}笔/天（v6.11: 5→4）")
        logger.info(f"✅ 冷却时间：{self.cooldown_hours}小时")
