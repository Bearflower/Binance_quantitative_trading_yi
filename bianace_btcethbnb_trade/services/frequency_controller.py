...
class FrequencyController:
    ...
    def __init__(self, db_manager=None):
        ...
        # v6.12 频率控制参数
        self.max_trades_per_day = 4  # 每日最大总交易数
        self.max_trades_per_symbol_per_day = 2  # 单品种每日最大交易数
        self.cooldown_hours = 12  # 同品种冷却期（小时）
        self.max_consecutive_losses = 5  # 连续亏损暂停阈值（笔数）
        self.max_daily_loss_amount = Decimal('25')  # 每日最大亏损限额（500U * 5% = 25U）
        self.total_capital = Decimal('500')  # 总资金
        
        logger.info("=" * 60)
        logger.info("频率控制器 v6.12 初始化完成")
        logger.info("=" * 60)
        logger.info(f"✅ 每日最大总交易数：{self.max_trades_per_day} 笔")
        logger.info(f"✅ 单品种每日最大交易数：{self.max_trades_per_symbol_per_day} 笔")
        logger.info(f"✅ 同品种冷却期：{self.cooldown_hours} 小时")
        logger.info(f"✅ 连续亏损暂停：{self.max_consecutive_losses} 笔")
        logger.info(f"✅ 每日最大亏损限额：{self.max_daily_loss_amount}U")
        logger.info("=" * 60)
    ...
    def check_trade_allowed(self, symbol: str) -> Tuple[bool, str]:
        ...
        if daily_total >= self.max_trades_per_day:
            logger.warning(f"⛔ 频率控制：当日已达交易上限（{daily_total}/{self.max_trades_per_day}）")
        ...
        if symbol_daily >= self.max_trades_per_symbol_per_day:
            logger.warning(f"⛔ 频率控制：{symbol} 当日已达交易上限（{symbol_daily}/{self.max_trades_per_symbol_per_day}）")
            return False, f"{symbol} 当日已达交易上限"
        
        # 3. 检查冷却期
        in_cooldown, cooldown_end = self._check_cooldown(symbol)
        if in_cooldown:
            logger.warning(f"⛔ 频率控制：{symbol} 处于冷却期，直到 {cooldown_end.strftime('%H:%M')}")
            return False, f"{symbol} 处于冷却期（{cooldown_end.strftime('%H:%M')} 结束）"
        
        # 4. 检查连续亏损
        consecutive_losses = self._get_consecutive_losses()
        if consecutive_losses >= self.max_consecutive_losses:
            logger.warning(f"⛔ 频率控制：连续亏损 {consecutive_losses} 笔，暂停交易 1 天")
            return False, f"连续亏损 {consecutive_losses} 笔，暂停交易"
        
        # 5. 检查每日亏损限额
        daily_pnl = self._get_daily_pnl()
        if daily_pnl < -self.max_daily_loss_amount:
            logger.warning(f"⛔ 频率控制：当日亏损 {abs(daily_pnl):.2f}U，超过限额 {self.max_daily_loss_amount}U")
            return False, f"当日亏损超限（{abs(daily_pnl):.2f}U/{self.max_daily_loss_amount}U）"
        
        logger.info(f"✅ 频率控制检查通过：{symbol} (今日 {daily_total}/{self.max_trades_per_day} 笔)")
    ...
    def record_trade(self, symbol: str, trade_time: datetime, 
        ...
        try:
            ...
            logger.info(f"✅ 交易记录已保存：{symbol} {direction} (盈亏：{pnl:.2f}U)")
    ...
    def _get_daily_total_trades(self) -> int:
        ...
        try:
            ...
            logger.error(f"查询今日交易数失败：{str(e)}")
            return 0
    ...
    def _get_symbol_daily_trades(self, symbol: str) -> int:
        ...
        try:
            ...
            logger.error(f"查询 {symbol} 今日交易数失败：{str(e)}")
            return 0
    ...
    def _get_daily_pnl(self) -> Decimal:
        ...
        try:
            ...
            logger.error(f"查询每日盈亏失败：{str(e)}")
            return Decimal('0')
    
    def reset_daily_stats(self):
        """重置每日统计（每日凌晨执行）"""
        logger.info("🔄 重置每日交易统计")
        # 数据库自动按日期统计，无需手动重置
        pass
...
def get_frequency_controller(db_manager=None) -> FrequencyController:
    """获取频率控制器实例（单例模式）"""
    global _global_controller
    if _global_controller is None:
        _global_controller = FrequencyController(db_manager)
