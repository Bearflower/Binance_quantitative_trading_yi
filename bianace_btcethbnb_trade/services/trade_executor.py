...
class TradeExecutor(BaseService):
    ...
    def _initialize(self):
        ...
        self.single_position_margin = Decimal(str(self.get_config_value(
        ...
        self.log_info(f"  默认杠杆：{self.default_leverage}x")
        self.log_info(f"  最大持仓：{self.max_positions}")
        self.log_info(f"  单仓保证金：{self.single_position_margin}U")
        ...
        try:
            ...
            for match in matches:
                ...
                try:
                    ...
                    self.log_warning(f"信号解析失败：{e}")

        except Exception as e:
            self.handle_error(e, context={'operation': 'parse_analysis_result'})
    ...
    async def execute_signal(self, signal: TradeSignal) -> Dict[str, Any]:
        ...
        Returns:
            执行结果
        """
        self.log_info(f"开始执行交易信号：{signal}")
        ...
        try:
            ...
            if signal.action == 'CLOSE':
                ...
                if not current_position or Decimal(current_position['positionAmt']) == 0:
                    self.log_warning(f"无持仓可平：{signal.symbol} {signal.position_side}")
            ...
            if signal.action == 'OPEN':
                ...
                self.log_info(f"需要保证金：{required_margin} USDT")
            ...
            self.log_info(f"下单成功：订单 ID={order['orderId']}")
            ...
            if signal.tp_price or signal.sl_price:
                try:
                    ...
                    if signal.tp_price:
                        ...
                        self.log_info(f"止盈单设置成功：订单 ID={tp_order_id}")
                    ...
                    if signal.sl_price:
                        ...
                        self.log_info(f"止损单设置成功：订单 ID={sl_order_id}")

                except Exception as e:
                    self.handle_error(e, context={'symbol': signal.symbol, 'operation': 'set_tp_sl'})
            ...
            self.handle_error(e, context={'symbol': signal.symbol, 'operation': 'execute_signal'})
    ...
    async def prepare_funds(self, required_amount: Decimal):
        ...
        """
        self.log_info(f"准备资金：需要 {required_amount} USDT")

        # 1. 检查 U 本位合约账户余额
        umfut_balance = self.api.get_umfut_balance('USDT')
        self.log_info(f"U 本位合约账户余额：{umfut_balance}")

        if umfut_balance >= required_amount:
            self.log_info("合约账户资金充足")
            return

        shortage = required_amount - umfut_balance
        self.log_info(f"资金缺口：{shortage}")

        # 2. 检查现货账户余额
        spot_balance = self.api.get_spot_balance('USDT')
        self.log_info(f"现货账户余额：{spot_balance}")

        if spot_balance >= shortage:
            self.log_info("从现货账户划转资金到合约账户")
        ...
        self.log_info(f"理财账户余额：{total_earn_balance}")
        ...
        # 执行赎回
        self.log_info(f"赎回理财产品：{shortage} USDT")
        ...
        for position in earn_positions:
            ...
            self.log_info(f"赎回成功：{redeem_amount} USDT")
...
def get_trade_executor(**kwargs) -> TradeExecutor:
    """
    获取全局交易执行器实例

    Args:
        **kwargs: 传递给 TradeExecutor 的参数
    ...
    if _executor is None:
        _executor = TradeExecutor(**kwargs)
    return _executor
