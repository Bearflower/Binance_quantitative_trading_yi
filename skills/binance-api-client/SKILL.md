...
# 3. 设置止损 - 使用条件单接口
stop_loss_order = api.place_pm_conditional_order(
    symbol="ETHUSDT",
    side="SELL",           # 多单平仓用 SELL
    position_side="BOTH",
    strategy_type="STOP_MARKET",  # 止损市单
    quantity=position_qty,        # 平仓数量
    stop_price=Decimal('2095.0'), # 触发价格
    reduce_only=True              # 只减仓，不开新仓
)
print(f"止损设置成功，策略 ID: {stop_loss_order['strategyId']}")
...
print(f"止盈设置成功，策略 ID: {take_profit_order['strategyId']}")
...
> **⭐ 完整接口文档**: 查看 [PM_ACCOUNT_API_GUIDE.md](PM_ACCOUNT_API_GUIDE.md) 获取详细的 PM 账户 API 接口文档，包含原始接口文档、接口对比表、特殊注意事项和实战调用流程。
...
7. **精度验证**：不要盲目信任 API 返回的精度数据，使用智能验证和修正
8. **频率控制**：批量下单时必须添加延迟控制，避免触发限流

## 实战经验

详细实战经验文档：[TRADING_EXPERIENCE.md](TRADING_EXPERIENCE.md)

**核心内容**：
- ✅ **精度问题处理**：BNBUSDT 精度修正、最小名义价值检查
- ✅ **频率限制控制**：智能延迟、重试机制、批量下单策略
- ✅ **常见错误码**：-1015、-1111、-2019 等错误的原因和解决方案
- ✅ **最佳实践**：错误处理、日志记录、监控告警、配置管理
- ✅ **故障排查**：订单失败、API 限流、精度数据不一致的排查步骤

**典型案例**：
1. BNBUSDT 数量精度从 0.001 修正为 0.01
2. 下单频率从 <1 秒 延长到 8 秒，成功率从 60% 提升到 95%
3. 智能精度验证避免 API 数据不一致问题
