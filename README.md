...
| 策略 | 策略ID | 版本 | 状态 | 说明 |
|------|--------|:----:|:----:|------|
| **MTPCS 趋势策略** | btc_eth | v6.30 | 🟢 运行中 | BTC/ETH/BNB/SOL/XRP 永续合约趋势 + 震荡双模式，动态ATR过滤+自适应仓位，V6.26 震荡反转风控三机制 + V6.27 时间平仓复核制 + V6.30 分批止盈调整（TP1 30% + TP2 40% + 剩余 30% 移动止损，硬止损全仓单保留） |
...
├── dashboard/            # 数据可视化看板
├── services/             # 独立服务
│   ├── kline_service/    # K线数据采集服务（独立部署，为所有策略提供K线数据）
...
- [文档索引](docs/README.md) - 完整文档目录（v6.30.0）
