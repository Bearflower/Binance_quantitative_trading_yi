"""
MTPCS激进策略适配器
采集主流币种趋势回调确认策略激进版(MTPCS激进)的周度表现数据

数据来源：
- trading.trade_records 表（strategy="MTPCS激进策略"）
- strategies/btc_eth_aggressive/config.yaml 配置文件
"""
...
class MTPCSAggressiveAdapter(BaseAdapter):
    """MTPCS激进策略（主流币趋势回调确认激进版）数据适配器"""

    strategy_id = "btc_eth_aggressive"
    strategy_name = "MTPCS激进策略"
    config_path = "strategies/btc_eth_aggressive/config.yaml"
    ...
    async def collect(self, week_offset: int = 0) -> StrategyReport:
        ...
        try:
            ...
            if not trades:
                logger.info("MTPCS激进策略本周无交易记录", strategy_id=self.strategy_id)
                return report
            ...
            logger.error("MTPCS激进策略数据采集异常", strategy_id=self.strategy_id, error=str(e))
