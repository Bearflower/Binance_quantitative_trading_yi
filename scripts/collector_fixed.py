...
class KlineCollector:
    ...
    async def collect_recent(
        ...
        interval_minutes = interval_minutes_map.get(interval, minutes)
        
        # 计算当前周期的开始时间（向下取整到周期边界）
        current_period_start = now.replace(second=0, microsecond=0)
        if interval in ["1h", "4h"]:
            current_period_start = current_period_start.replace(minute=0)
        elif interval == "1d":
            current_period_start = current_period_start.replace(minute=0, hour=0)
        elif interval == "15m":
            current_period_start = current_period_start.replace(
                minute=(current_period_start.minute // 15) * 15
            )
        elif interval == "5m":
            current_period_start = current_period_start.replace(
                minute=(current_period_start.minute // 5) * 5
            )
        
        # 上一个完整周期的结束时间
        end_time = int(current_period_start.timestamp() * 1000)
        # 上一个完整周期的开始时间
        start_time = int(
            (current_period_start - timedelta(minutes=interval_minutes)).timestamp() * 1000
        )

        logger.info(
            f"采集 {symbol} {interval} 时间范围: "
        ...
        if klines:
            ...
            if filtered_klines:
                ...
                logger.info(
                    f"采集 {symbol} {interval}：获取{len(klines)}条，"
                    f"过滤后{len(filtered_klines)}条，存储{stored}条"
                )
                return stored
            else:
                logger.warning(
            ...
            logger.warning(f"采集 {symbol} {interval}：未获取到数据")
