...
- [月度资金分配需求文档](requirements/capital_allocation/月度资金分配需求文档.md) - 每月按收益率排名分配各策略资金额度（分配算法、数据存储、每日净资产重算 allocated_amount）
- [策略资金限额强制执行需求文档](requirements/capital_allocation/策略资金限额强制执行需求文档.md) - 限额执行链（保证金口径、DB→config→静态优先级链、持仓基线重建、AC 验收标准）

### 组合级行情熔断

- [单边行情空头熔断需求文档](requirements/market_circuit_breaker/单边行情空头熔断需求文档.md) - 组合级双向熔断（二期：1h 瞬时 + 12h 累计；轧空拦空+轧多拦多+拦慢牛阴影）、分池双指数、data-backend 每日03分指数job、三策略接入、07建表（含 12h 三列）

### MTPCS 持仓归属

- [MTPCS双策略持仓归属互斥（方案C）需求文档](requirements/mtpcs/MTPCS双策略持仓归属互斥（方案C）需求文档.md) - 共享 PM 账户下原版+激进版持仓归属互斥（边界A归属兜底 + 边界B持仓期互斥 + 三处归属守卫 + `filter_owned_positions` 上报归属过滤），根治互挂保护单/互相误平和看板误报归属，已上线灰度
