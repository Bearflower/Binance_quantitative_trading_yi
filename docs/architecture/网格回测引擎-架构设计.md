...
async def _simulate(self, week_start, week_end, symbols):
    ...
    # 5. 执行回测
    result = engine.run(klines, current_params)
    result.scenario_name = "当前配置"

    # 6. 转换为 SimulationMetrics 列表（保持接口兼容）
    return [self._backtest_result_to_simulation(result)]
```
...
def _backtest_result_to_simulation(self, result: BacktestResult) -> SimulationMetrics:
    """将 BacktestResult 转换为 SimulationMetrics（保持接口兼容）"""
    return SimulationMetrics(
        scenario_name=result.scenario_name,
        symbol=result.symbol,
        market_state="",  # 回测模式下不单独标记市况
        grid_count=0,     # 从参数中提取
        grid_spacing=0,
        price_range_low=0,
        price_range_high=0,
        profit_rate_per_fill=0,
        estimated_fills_weekly=result.fill_count,
        estimated_profit_weekly=result.total_pnl,
        confidence=0.8,   # 回测模式置信度较高
    )
```
