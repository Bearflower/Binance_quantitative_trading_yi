...
### 第4层：安全审批与差分更新（人工在环 + 自动审批）

**目标**：杜绝AI幻觉导致的灾难性参数，所有变更必须经确认后方可生效。支持人工审批和自动审批两种模式。
...
- **确认机制**：
  - 人工审批模式（默认）：管理员在群里回复特定关键词（如"/confirm HRS_20260621"）触发脚本执行，或通过HTTP回调端点确认。
  - 自动审批模式（auto-apply）：在 `config.yaml` 中将 `approval.auto_apply.enabled` 设置为 `true` 后，AI 调优建议在推送飞书通知的同时自动写入覆盖层并生效，无需人工介入（opt-in 模式，默认关闭）。
...
6. **推送通知与生效**：通过飞书发送变更清单，根据审批模式决定后续流程。人工审批模式下等待人工确认（超时未确认则不生效）；auto-apply 模式下自动写入覆盖层并发送自动应用通知。
