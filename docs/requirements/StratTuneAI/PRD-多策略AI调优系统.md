...
| 维度 | 第一期覆盖 | 后续扩展 |
|------|-----------|----------|
| 策略 | MTPCS策略（btc_eth）+ 新币做空策略（new_coin） | HRS反转、网格交易 |
| 调优频率 | 仅周度调优 | 月度调优（可选） |
...
| 决策项 | 结论 |
|--------|------|
| 预留方式 | **现在预留**——在 `config.yaml` 中注册 HRS 策略配置和白名单 |
| 状态 | 标记为 `enabled: false`，调度器跳过 |
| 上线条件 | HRS 策略上线后，将 `enabled` 改为 `true`，编写 Prompt 模板即可 |
