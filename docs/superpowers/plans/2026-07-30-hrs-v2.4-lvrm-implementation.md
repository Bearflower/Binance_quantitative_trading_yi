...
| 版本 | 日期 | 说明 |
|------|------|------|
| V2.4 | 2026-07-30 | 本文档初始版本，LV-RM 模块实现计划 |
| V2.8 | 2026-09-01 | 趋势过滤配置值已更新：`long.max_deviation` 从 0.97 收紧至 0.99，`short.max_deviation` 从 1.03 收紧至 1.01，新增 `ema_slope` 配置段（见 `config.yaml` 中 `scoring.trend_filter` 配置段） |