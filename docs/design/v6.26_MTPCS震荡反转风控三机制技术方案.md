...
| 项目 | 内容 |
|------|------|
| 方案版本 | v1.1（v6.28 同步机制③a 实现修正） |
| 对应需求 | `docs/requirements/btc_eth/v6.26_MTPCS震荡反转风控三机制需求文档.md` |
| 代码基线 | v6.26 → v6.28（机制③a 量价确认已修正为半区校验，见 3.6） |
...
def _check_volume_confirm(
    ...
    #
    # shrink_ratio = cfg.get('shrink_ratio', 1.1)
    # # 价格方向确认（v6.28 修正：半区校验，替代「穿越中轨」）
...
```yaml
    ...
    ranging_strategy:
      ...
      volume_confirm:
        enabled: true
        shrink_ratio: 1.1            # 缩量判定：当前 4h 成交量 < shrink_ratio × Volume_MA(20)（v6.28 放宽 1.0→1.1）
