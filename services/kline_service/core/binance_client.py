...
class BinanceClient:
    ...
    使用期货 API (fapi.binance.com) 获取 K 线数据
    """
            ...
            try:
                ...
                async with self.session.request(
                    ...
                    if response.status == 200:
                        ...
                        # 400 错误通常是客户端请求错误（如无效标的），使用 WARNING 级别
                        if 400 <= response.status < 500:
                            logger.warning(
                                f"币安 API 请求失败（客户端错误）：{response.status} - {error_text}"
                            )
                        else:
                            logger.error(
                                f"币安 API 请求失败：{response.status} - {error_text}"
                            )
                        return None
