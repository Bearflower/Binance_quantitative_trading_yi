...
async def get_new_perpetual_contracts(
...
async def main():
    ...
    # 如果没有配置API密钥，使用空字符串（公共接口不需要）
    if not api_key or not api_secret:
        logger.warning("未配置API密钥，将使用公共接口（部分功能可能受限）")
    ...
    # 创建币安客户端（使用公共接口，不需要真实密钥）
    async with BinanceClient(
        api_key=api_key,
        api_secret=api_secret,
        testnet=False,
        use_unified_account=False  # 使用普通合约账户端点
    ) as client:
        # 获取新上线的永续合约
        new_contracts = await get_new_perpetual_contracts(
            client=client,
